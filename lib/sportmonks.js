...
function removeInclude(includeValue, blocked) {
  if (!includeValue) return includeValue;
  const parts = String(includeValue).split(';').map(x => x.trim()).filter(Boolean);
  const filtered = parts.filter(x => x.toLowerCase() !== String(blocked).toLowerCase());
...
async function rawRequest(path, params = {}) {
  ...
  if (!res.ok) {
    const err = new Error(`Sportmonks ${res.status}: ${text.slice(0, 500)}`);
...
async function request(path, params = {}) {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    try {
      const json = await rawRequest(path, current);
      if (removed.length) json.__removedIncludes = removed;
      return json;
    } catch (err) {
      const body = String(err?.body || err?.message || '');
      const match = body.match(/access to the ['\"]([^'\"]+)['\"] include/i);

      if (err?.status === 403 && match && current.include) {
        const blocked = match[1];
        const nextInclude = removeInclude(current.include, blocked);
        if (nextInclude === current.include) throw err;
        removed.push(blocked);
        current = { ...current, include: nextInclude || undefined };
        continue;
      }
  ...
  throw new Error('Sportmonks include fallback sınırı aşıldı.');
}
